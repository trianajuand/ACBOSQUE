...
5. Consulta el perfil `Inversionista` asociado.
6. Mapea entidades a `PerfilInversionistaDTO`.
7. Audita `PERFIL_CONSULTADO`.
8. Frontend actualiza formularios y paneles.
...
## Base de datos
Tabla `usuario` contiene identidad/acceso general. Tabla `inversionista` contiene datos propios del inversionista: perfil, preferencias, notificaciones, Alpaca, Stripe y premium. No se exponen columnas sensibles como `contrasenia`.
...
## Dependencias
Depende de login/JWT y de las entidades `Usuario` e `Inversionista`.
