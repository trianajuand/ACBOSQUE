...
3. `OrquestadorRegistro` invoca `IIntegracionAlpaca.crearCuenta(usuario, inversionista)`.
4. `AlpacaAdapter` arma payload sandbox y llama Broker API.
5. Si Alpaca retorna `id`, se guarda en `inversionista.alpacaAccountId`.
...
- `backend/.../autenticacion/repository/InversionistaRepository.java`
- `backend/.../trazabilidad/interfaces/IAuditLog.java`

## Backend
El adaptador usa `RestTemplate`, Basic Auth para Broker API y propiedades `alpaca.broker.*`. El orquestador actualiza el perfil `inversionista` y audita `REGISTRO_EXITOSO` o `REGISTRO_FALLO_ALPACA`.

## Frontend
No hay pantalla específica. El efecto se dispara después del flujo de verificación o pago.

## Base de datos
Tabla `inversionista`: columnas `alpaca_account_id` y `pendiente_cuenta_alpaca`.
...
- [ ] Si Alpaca responde con id se guarda `alpacaAccountId` en `inversionista`.
- [ ] Si Alpaca falla se marca la cuenta local como pendiente.

## Notas
La creación on-demand también existe al enviar órdenes US si el inversionista aún no tiene `alpacaAccountId`.
