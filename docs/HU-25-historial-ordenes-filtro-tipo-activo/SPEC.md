...
## Titulo
Historial de ordenes filtrado por tipo y activo.

## Descripcion
Como inversionista autenticado
Quiero filtrar historial por tipo de orden o activo
Para encontrar operaciones relevantes rapidamente.

## Contexto
HU-25 forma parte del bloque de historial con filtros. El endpoint de historial acepta tipo de orden y simbolo, y el dashboard Angular expone ambos filtros.
...
## Backend
`GET /api/ordenes/historial` acepta `tipoOrden` y `simbolo`. `OrdenService.obtenerHistorialOrdenes` valida el enum de tipo y filtra el activo normalizado en mayusculas.
...
## API / Endpoints
- `GET /api/ordenes/historial?tipoOrden={MARKET|LIMIT|STOP_LOSS|TAKE_PROFIT}&simbolo={ticker}`
...
- [x] Filtros de tipo y activo conectados en frontend.
