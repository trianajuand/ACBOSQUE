...
## Contexto
El registro es la puerta de entrada del servicio de Autenticación. La implementación actual exige un flujo Angular por fases y persiste la información principal en la entidad `Usuario`, dejando la cuenta en estado `PENDIENTE_VERIFICACION` hasta validar el código de correo. El diseño sigue RF-01, EC-10 y las reglas de seguridad de persistir códigos en BD.
...
## Backend
`RegistroController` expone el registro público. `RegistroService.iniciarRegistro` valida duplicidad, mapea el DTO a `Usuario`, asigna rol y estado, codifica contraseña, guarda preferencias iniciales, plan y datos financieros, y delega la generación de código a `MFAService`.
...
## Base de datos
Tabla `usuario`: columnas de acceso, identidad, dirección, perfil financiero, preferencias, plan y estado. Tabla `codigo_verificacion`: código de registro, tipo `REGISTRO`, expiración y flag `usado`.
